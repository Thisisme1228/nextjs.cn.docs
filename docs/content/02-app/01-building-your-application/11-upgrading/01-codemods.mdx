...
This codemod moves transforms imports from `next/server` to `next/og` for usage of [Dynamic OG Image Generation](/docs/content/app/building-your-application/optimizing/metadata#dynamic-image-generation).
...
export default function Home() {
  return (
    <div>
      <img1 src="/test.jpg" width="200" height="300" />
      <img2 src="/test.png" width="500" height="400" />
    </div>
  );
}
```
...
Remove `<a>` tags inside [Link Components](/docs/content/app/api-reference/components/link), or add a `legacyBehavior` prop to Links that cannot be auto-fixed.

</AppOnly>

<PagesOnly>

Remove `<a>` tags inside [Link Components](/docs/content/pages/api-reference/components/link), or add a `legacyBehavior` prop to Links that cannot be auto-fixed.
