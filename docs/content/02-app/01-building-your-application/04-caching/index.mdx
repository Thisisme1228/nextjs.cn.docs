...
<img
  ...
  srcDark="/deduplicated-fetch-requests.avif"
  width="100%"
  height="auto"
/>
...
<img
  ...
  srcDark="/request-memoization.avif"
  width="100%"
  height="auto"
/>
...
<img
  ...
  srcDark="/time-based-revalidation.avif"
  width="100%"
  height="auto"
/>
...
<img
  ...
  srcDark="/on-demand-revalidation.avif"
  width="100%"
  height="auto"
/>
...
>
> To learn more, see the [Server Components](/docs/content/app/building-your-application/rendering/server-components) documentation.
...
<img
  ...
  srcDark="/static-and-dynamic-routes.avif"
  width="100%"
  height="auto"
/>

Learn more about [static and dynamic rendering](/docs/content/app/building-your-application/rendering/server-components#server-rendering-strategies).
...
- **[Revalidating Data](/docs/content/app/building-your-application/caching#revalidating)**: Revalidating the [Data Cache](#data-cache), will in turn invalidate the Router Cache by re-rendering components on the server and caching the new render output.
...
When a user navigates between routes, Next.js caches the visited route segments and [prefetches](/docs/content/app/building-your-application/routing/linking-and-navigating#2-prefetching) the routes the user is likely to navigate to.This results in instant back/forward navigation, no full-page reload between navigations, and preservation of React state and browser state.

With the Router Cache:

- **Layouts** are cached and reused on navigation ([partial rendering](/docs/content/app/building-your-application/routing/linking-and-navigating#4-partial-rendering)).
- **Loading states** are cached and reused on navigation for [instant navigation](/docs/content/app/building-your-application/routing/loading-ui-and-streaming#instant-loading-states).
- **Pages** are not cached by default, but are reused during browser backward and forward navigation. You can enable caching for page segments by using the experimental [`staleTimes`](/docs/content/app/api-reference/next-config-js/staleTimes) config option.
...
- **Automatic Invalidation Period**: The cache of layouts and loading states is automatically invalidated after a specific time. The duration depends on how the resource was [prefetched](/docs/content/app/api-reference/components/link#prefetch), and if the resource was [statically generated](/docs/content/app/building-your-application/rendering/server-components#static-rendering-default):
...
> **Good to know**: The experimental [`staleTimes`](/docs/content/app/api-reference/next-config-js/staleTimes) config option can be used to adjust the automatic invalidation times mentioned above.
...
- In a **Server Action**:
  - Revalidating data on-demand by path with ([`revalidatePath`](/docs/content/app/api-reference/functions/revalidatePath)) or by cache tag with ([`revalidateTag`](/docs/content/app/api-reference/functions/revalidateTag))
  - Using [`cookies.set`](/docs/content/app/api-reference/functions/cookies#cookiessetname-value-options) or [`cookies.delete`](/docs/content/app/api-reference/functions/cookies#deleting-cookies) invalidates the Router Cache to prevent routes that use cookies from becoming stale (e.g. authentication).
- Calling [`router.refresh`](/docs/content/app/api-reference/functions/use-router) will invalidate the Router Cache and make a new request to the server for the current route.

### Opting out

As of Next.js 15, page segments are opted out by default.

> **Good to know:** You can also opt out of [prefetching](/docs/content/app/building-your-application/routing/linking-and-navigating#2-prefetching) by setting the `prefetch` prop of the `<Link>` component to `false`.
...
- To immediately invalidate the Data Cache and Router cache, you can use [`revalidatePath`](#revalidatepath) or [`revalidateTag`](#fetch-optionsnexttags-and-revalidatetag) in a [Server Action](/docs/content/app/building-your-application/data-fetching/server-actions-and-mutations).
- Revalidating the Data Cache in a [Route Handler](/docs/content/app/building-your-application/routing/route-handlers) **will not** immediately invalidate the Router Cache as the Route Handler isn't tied to a specific route. This means Router Cache will continue to serve the previous payload until a hard refresh, or the automatic invalidation period has elapsed.
...
Learn more about the [`<Link>` component](/docs/content/app/api-reference/components/link).
...
See the [`useRouter` hook](/docs/content/app/api-reference/functions/use-router) API reference.
...
See the [`fetch` API Reference](/docs/content/app/api-reference/functions/fetch) for more options.
...
1. When using `fetch` or [`unstable_cache`](/docs/content/app/api-reference/functions/unstable_cache), you have the option to tag cache entries with one or more tags.
...
1. [Route Handlers](/docs/content/app/building-your-application/routing/route-handlers) - to revalidate data in response of a third party event (e.g. webhook). This will not invalidate the Router Cache immediately as the Router Handler isn't tied to a specific route.
2. [Server Actions](/docs/content/app/building-your-application/data-fetching/server-actions-and-mutations) - to revalidate data after a user action (e.g. form submission). This will invalidate the Router Cache for the associated route.
...
1. [Route Handlers](/docs/content/app/building-your-application/routing/route-handlers) - to revalidate data in response to a third party event (e.g. webhook).
2. [Server Actions](/docs/content/app/building-your-application/data-fetching/server-actions-and-mutations) - to revalidate data after a user interaction (e.g. form submission, clicking a button).

See the [`revalidatePath` API reference](/docs/content/app/api-reference/functions/revalidatePath) for more information.
...
See the [`cookies`](/docs/content/app/api-reference/functions/cookies) API reference.
...
See the [Route Segment Config](/docs/content/app/api-reference/file-conventions/route-segment-config) documentation for more options.

### `generateStaticParams`

For [dynamic segments](/docs/content/app/building-your-application/routing/dynamic-routes) (e.g. `app/blog/[slug]/page.js`), paths provided by `generateStaticParams` are cached in the Full Route Cache at build time. At request time, Next.js will also cache paths that weren't known at build time the first time they're visited.
...
To statically render all paths the first time they're visited, return an empty array (no paths will be rendered at build time) or utilize [`export const dynamic = 'force-static'`](/docs/content/app/api-reference/file-conventions/route-segment-config#dynamic):
...
To disable caching at request time, add the `export const dynamicParams = false` option in a route segment. When this config option is used, only paths provided by `generateStaticParams` will be served, and other routes will 404 or match (in the case of [catch-all routes](/docs/content/app/building-your-application/routing/dynamic-routes#catch-all-segments)).
