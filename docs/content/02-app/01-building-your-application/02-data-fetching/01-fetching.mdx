...
For most cases, where you don't need real-time data (e.g. polling), you can fetch data on the server with [Server Components](/docs/content/app/building-your-application/rendering/server-components). There are a few benefits to doing this:
...
- Data requests can be [cached](/docs/content/app/building-your-application/data-fetching/caching-and-revalidating#caching) and [revalidated](/docs/content/app/building-your-application/data-fetching/caching-and-revalidating#revalidating-data).
...
Next.js extends the native [`fetch` Web API](https://developer.mozilla.org/docs/Web/API/Fetch_API) `fetch` to allow you to configure the [caching](/docs/content/app/building-your-application/data-fetching/caching-and-revalidating#fetch-requests) and [revalidating](/docs/content/app/building-your-application/data-fetching/caching-and-revalidating#revalidating-data) behavior for each fetch request on the server. You can use `fetch` in [Server Components](/docs/content/app/building-your-application/rendering/server-components), [Route Handlers](/docs/content/app/building-your-application/routing/route-handlers), and [Server Actions](/docs/content/app/building-your-application/data-fetching/server-actions-and-mutations). For example:
...
By default, `fetch` requests retrieve fresh data. Using it will cause a whole route to be [dynamically rendered](/docs/content/app/building-your-application/rendering/server-components#dynamic-rendering) and data will not be cached.

You can cache `fetch` requests by setting the `cache` option to `force-cache`. This means data will be cached, and the component will be [statically rendered](/docs/content/app/building-your-application/rendering/server-components#static-rendering-default):

```js
fetch("https://...", { cache: "force-cache" });
```

Alternatively, if using [PPR](/docs/content/app/building-your-application/rendering/partial-prerendering), we recommend wrapping components that use `fetch` requests in Suspense boundaries. This will ensure only the component that uses `fetch` is dynamically rendered and streamed in, rather than the entire page:
...
See the [caching and revalidating](/docs/content/app/building-your-application/data-fetching/caching-and-revalidating) docs for more information.

> **Good to know:** In Next.js 14 and earlier, `fetch` requests were cached by default. See the [upgrade guide](/docs/content/app/building-your-application/upgrading/version-15) for more information.
...
Learn more about [request memoization](/docs/content/app/building-your-application/caching#request-memoization).

## ORMs and Database Clients

You can call your ORM or database client in [Server Components](/docs/content/app/building-your-application/rendering/server-components), [Route Handlers](/docs/content/app/building-your-application/routing/route-handlers), and [Server Actions](/docs/content/app/building-your-application/data-fetching/server-actions-and-mutations).
...
You can also configure the caching and revalidating behavior of these requests using the experimental [`unstable_cache`](/docs/content/app/api-reference/functions/unstable_cache) and [`unstable_noStore`](/docs/content/app/api-reference/functions/unstable_noStore) APIs.
...
If you need to create API endpoints, Next.js supports [Route Handlers](/docs/content/app/building-your-application/routing/route-handlers). Route Handlers execute on the server and prevent sensitive information from being exposed to the client (e.g. API credentials).
...
See the [Route Handler](/docs/content/app/building-your-application/routing/route-handlers) docs for more examples.
...
If you have nested components, and each component fetches its own data, then data fetching will happen sequentially if those data requests are not [memoized](/docs/content/app/building-your-application/caching#request-memoization).
...
You can use [`loading.js`](/docs/content/app/building-your-application/routing/loading-ui-and-streaming) (for route segments) or [React `<Suspense>`](/docs/content/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense) (for nested components) to show an instant loading state while React streams in the result.
...
In addition, you can add a [Suspense Boundary](/docs/content/app/building-your-application/routing/loading-ui-and-streaming) to break up the rendering work and show part of the result as soon as possible.
...
With this approach, you can eagerly fetch data, cache responses, and guarantee that this data fetching [only happens on the server](/docs/content/app/building-your-application/rendering/composition-patterns#keeping-server-only-code-out-of-the-client-environment).

The `utils/get-item` exports can be used by Layouts, Pages, or other components to give them control over when an item's data is fetched.

> **Good to know:**
>
> - We recommend using the [`server-only` package](/docs/content/app/building-your-application/rendering/composition-patterns#keeping-server-only-code-out-of-the-client-environment) to make sure server data fetching functions are never used on the client.
