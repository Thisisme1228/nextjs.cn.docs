...
Since Next.js does not have access to remote files during the build process, you'll need to provide the [`width`](/docs/content/app/api-reference/components/image#width), [`height`](/docs/content/app/api-reference/components/image#height) and optional [`blurDataURL`](/docs/content/app/api-reference/components/image#blurdataurl) props manually.
...
Learn more about [`remotePatterns`](/docs/content/app/api-reference/components/image#remotepatterns) configuration. If you want to use relative URLs for the image `src`, use a [`loader`](/docs/content/app/api-reference/components/image#loader).
...
> Learn more about [`remotePatterns`](/docs/content/app/api-reference/components/image#remotepatterns) configuration.
...
You can define a loader per-image with the [`loader` prop](/docs/content/app/api-reference/components/image#loader), or at the application level with the [`loaderFile` configuration](/docs/content/app/api-reference/components/image#loaderfile).
...
The LCP element is typically the largest image or text block visible within the viewport of the page. When you run `next dev`, you'll see a console warning if the LCP element is an `<img>` without the `priority` property.
...
export default function Page() {
  return <img src={profilePic} alt="Picture of the author" priority />;
}
```

See more about priority in the [`next/image` component documentation](/docs/content/app/api-reference/components/image#priority).
...
2. Manually, by including a [`width`](/docs/content/app/api-reference/components/image#width) and [`height`](/docs/content/app/api-reference/components/image#height) property used to determine the image's aspect ratio.
3. Implicitly, by using [fill](/docs/content/app/api-reference/components/image#fill) which causes the image to expand to fill its parent element.
...
- Use `className` or `style`, not `styled-jsx`.
  - In most cases, we recommend using the `className` prop. This can be an imported [CSS Module](/docs/content/app/building-your-application/styling/css), a [global stylesheet](/docs/content/app/building-your-application/styling/css#global-styles), etc.
  - You can also use the `style` prop to assign inline styles.
  - You cannot use [styled-jsx](/docs/content/app/building-your-application/styling/css-in-js) because it's scoped to the current component (unless you mark the style as `global`).
...
[**View all properties available to the `next/image` component.**](/docs/content/app/api-reference/components/image)

## Configuration

The `next/image` component and Next.js Image Optimization API can be configured in the [`next.config.js` file](/docs/content/app/api-reference/next-config-js). These configurations allow you to [enable remote images](/docs/content/app/api-reference/components/image#remotepatterns), [define custom image breakpoints](/docs/content/app/api-reference/components/image#devicesizes), [change caching behavior](/docs/content/app/api-reference/components/image#caching-behavior) and more.

[**Read the full image configuration documentation for more information.**](/docs/content/app/api-reference/components/image#configuration-options)
