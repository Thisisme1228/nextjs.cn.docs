...
An object containing the [dynamic route parameters](/docs/content/app/building-your-application/routing/dynamic-routes) object from the root segment down to the segment `generateImageMetadata` is called from.
...
This example uses the `params` object and external data to generate multiple [Open Graph images](/docs/content/app/api-reference/file-conventions/metadata/opengraph-image) for a route segment.
