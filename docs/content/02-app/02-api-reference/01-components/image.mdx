...
> **Good to know**: If you are using a version of Next.js prior to 13, you'll want to use the [next/legacy/image](/docs/content/pages/api-reference/components/image-legacy) documentation since the component was renamed.

</PagesOnly>

This API reference will help you understand how to use [props](#props) and [configuration options](#configuration-options) available for the Image Component. For features and usage, please see the [Image Component](/docs/content/app/building-your-application/optimizing/images) page.
...
- A [statically imported](/docs/content/app/building-your-application/optimizing/images#local-images) image file
...
Required, except for [statically imported images](/docs/content/app/building-your-application/optimizing/images#local-images) or images with the [`fill` property](#fill).
...
The `<img />` component accepts a number of additional properties beyond those which are required. This section describes the most commonly-used properties of the Image component. Find details about more rarely-used properties in the [Advanced Props](#advanced-props) section.
...
> **Good to know**: Using props like `loader`, which accept a function, requires using [Client Components](/docs/content/app/building-your-application/rendering/client-components) to serialize the provided function.
...
When `blur`, the [`blurDataURL`](#blurdataurl) property will be used as the placeholder. If `src` is an object from a [static import](/docs/content/app/building-your-application/optimizing/images#local-images) and the imported image is `.jpg`, `.png`, `.webp`, or `.avif`, then `blurDataURL` will be automatically populated, except when the image is detected to be animated.
...
In some cases, you may need more advanced usage. The `<img />` component optionally accepts the following advanced properties.
...
export default function ProfileImage() {
  return <img src="..." style={imageStyle} />;
}
```
...
<img onLoadingComplete={(img) => console.log(img.naturalWidth)} />
```
...
> **Good to know**: Using props like `onLoadingComplete`, which accept a function, requires using [Client Components](/docs/content/app/building-your-application/rendering/client-components) to serialize the provided function.
...
```jsx
<img onLoad={(e) => console.log(e.target.naturalWidth)} />
```
...
> **Good to know**: Using props like `onLoad`, which accept a function, requires using [Client Components](/docs/content/app/building-your-application/rendering/client-components) to serialize the provided function.
...
```jsx
<img onError={(e) => console.error(e.target.id)} />
```
...
> **Good to know**: Using props like `onError`, which accept a function, requires using [Client Components](/docs/content/app/building-your-application/rendering/client-components) to serialize the provided function.
...
When providing the `src` prop to the `<img>` component, both the `srcset` and `src` attributes are generated automatically for the resulting `<img>`.
...
For example, when upgrading an existing website from `<img>` to `<img>`, you may wish to maintain the same `src` attribute for SEO purposes such as image ranking or avoiding recrawl.

```jsx filename="input.js"
<img src="/me.jpg" overrideSrc="/override.jpg" />
```
...
Other properties on the `<img />` component will be passed to the underlying
...
- [Custom Image Loader Configuration](/docs/content/app/api-reference/next-config-js/images#example-loader-configuration)

<AppOnly>

> **Good to know**: Customizing the image loader file, which accepts a function, requires using [Client Components](/docs/content/app/building-your-application/rendering/client-components) to serialize the provided function.
...
You can configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](/docs/content/app/building-your-application/optimizing/images#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
If you need to change the caching behavior per image, you can configure [`headers`](/docs/content/app/api-reference/next-config-js/headers) to set the `Cache-Control` header on the upstream image (e.g. `/some-asset.jpg`, not `/_next/image` itself).
...
The default [loader](#loader) does not optimize SVG images for a few reasons. First, SVG is a vector format meaning it can be resized losslessly. Second, SVG has many of the same features as HTML/CSS, which can lead to vulnerabilities without proper [Content Security Policy (CSP) headers](/docs/content/app/api-reference/next-config-js/headers#content-security-policy).
...
If you want to display a different image for light and dark mode, you can create a new component that wraps two `<img>` components and reveals the correct one based on a CSS media query.
...
type Props = Omit<imgProps, "src" | "priority" | "loading"> & {
  srcLight: string;
  srcDark: string;
};
...
const ThemeImage = (props: Props) => {
  ...
  return (
    <>
      <img {...rest} src={srcLight} className={styles.imgLight} />
      <img {...rest} src={srcDark} className={styles.imgDark} />
    </>
  );
};
```
...
| `v13.0.0`  | The `next/image` import was renamed to `next/legacy/image`. The `next/future/image` import was renamed to `next/image`. A [codemod is available](/docs/content/app/building-your-application/upgrading/codemods#next-image-to-legacy-image) to safely and automatically rename your imports. `<span>` wrapper removed. `layout`, `objectFit`, `objectPosition`, `lazyBoundary`, `lazyRoot` props removed. `alt` is required. `onLoadingComplete` receives reference to `img` element. Built-in loader config removed. |
