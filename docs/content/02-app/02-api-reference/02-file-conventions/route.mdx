...
Currently, the only value of `context` is `params`, which is an object containing the [dynamic route parameters](/docs/content/app/building-your-application/routing/dynamic-routes) for the current route.
...
Route Handlers can extend the Web Response API by returning a `NextResponse` object. This allows you to easily set cookies, headers, redirect, and rewrite. [View the API reference](/docs/content/app/api-reference/functions/next-response).
