...
[Turbopack](/docs/content/architecture/turbopack) (beta), our new bundler, which is being tested and stabilized in Next.js, helps speed up local iterations while working on your application.
...
Both of these values are [**compressed with gzip**](/docs/content/app/api-reference/next-config-js/compress). The first load is indicated by green, yellow, or red. Aim for green for performant applications.
...
For more information on the other options, check out our [ESLint](/docs/content/app/building-your-application/configuring/eslint) configuration documentation.
