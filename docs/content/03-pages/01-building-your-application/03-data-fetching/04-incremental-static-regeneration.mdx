...
> **Good to know**: The [`edge` runtime](/docs/content/pages/api-reference/edge) is currently not compatible with ISR, although you can leverage `stale-while-revalidate` by setting the `cache-control` header manually.
...
Next, add the secret as an [Environment Variable](/docs/content/pages/building-your-application/configuring/environment-variables) to your application. Finally, create the revalidation API Route:
...
When running locally with `next dev`, `getStaticProps` is invoked on every request. To verify your on-demand ISR configuration is correct, you will need to create a [production build](/docs/content/pages/api-reference/next-cli#build) and start the [production server](/docs/content/pages/api-reference/next-cli#production):
...
Incremental Static Regeneration (ISR) works on [self-hosted Next.js sites](/docs/content/pages/building-your-application/deploying#self-hosting) out of the box when you use `next start`.

Learn more about [self-hosting Next.js](/docs/content/pages/building-your-application/deploying#self-hosting).
