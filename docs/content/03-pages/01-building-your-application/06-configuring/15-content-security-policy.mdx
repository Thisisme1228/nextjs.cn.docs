...
[Middleware](/docs/content/app/building-your-application/routing/middleware) enables you to add headers and generate nonces before the page renders.
...
By default, Middleware runs on all requests. You can filter Middleware to run on specific paths using a [`matcher`](/docs/content/app/building-your-application/routing/middleware#matcher).
...
You can now read the nonce from a [Server Component](/docs/content/app/building-your-application/rendering/server-components) using [`headers`](/docs/content/app/api-reference/functions/headers):
...
For applications that do not require nonces, you can set the CSP header directly in your [`next.config.js`](/docs/content/app/api-reference/next-config-js) file:
