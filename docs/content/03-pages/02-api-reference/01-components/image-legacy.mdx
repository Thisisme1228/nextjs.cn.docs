...
View the **new** [`next/image` API Reference](/docs/content/pages/api-reference/components/image)
...
The `<img />` component requires the following properties.

### src

Must be one of the following:

- A [statically imported](/docs/content/pages/building-your-application/optimizing/images#local-images) image file
...
The `width` property is required, except for [statically imported images](/docs/content/pages/building-your-application/optimizing/images#local-images), or those with `layout="fill"`.
...
The `height` property is required, except for [statically imported images](/docs/content/pages/building-your-application/optimizing/images#local-images), or those with `layout="fill"`.
...
When `blur`, the [`blurDataURL`](#blurdataurl) property will be used as the placeholder. If `src` is an object from a [static import](/docs/content/pages/building-your-application/optimizing/images#local-images) and the imported image is `.jpg`, `.png`, `.webp`, or `.avif`, then `blurDataURL` will be automatically populated.
...
const Example = () => {
  ...
  return (
    <div ref={lazyRoot} style={{ overflowX: "scroll", width: "500px" }}>
      <img lazyRoot={lazyRoot} src="/one.jpg" width="500" height="500" />
      <img lazyRoot={lazyRoot} src="/two.jpg" width="500" height="500" />
    </div>
  );
};
```
...
You can configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](/docs/content/pages/building-your-application/optimizing/images#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
If you need to change the caching behavior per image, you can configure [`headers`](/docs/content/pages/api-reference/next-config-js/headers) to set the `Cache-Control` header on the upstream image (e.g. `/some-asset.jpg`, not `/_next/image` itself).
